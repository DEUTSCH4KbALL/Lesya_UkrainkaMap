...
document.addEventListener("DOMContentLoaded", function () {
    ...
    signInForm.addEventListener('submit', function (event) {
        ...
        const name = signInForm.querySelector('input[type="text"]').value; // Отримуємо ім'я
        const email = signInForm.querySelector('input[type="email"]').value; // Отримуємо email
        const password = signInForm.querySelector('input[type="password"]').value; // Отримуємо пароль

        // Отримуємо збережені дані з localStorage
        const storedName = localStorage.getItem('userName'); // Отримуємо збережене ім'я
        ...
        if (name === storedName && email === storedEmail && password === storedPassword) {
            console.log('Дані співпадають, перенаправляємо на main.html');
    ...
    // Функція для відображення даних профілю на сторінці реєстрації (якщо це необхідно)
    displayProfileData();
